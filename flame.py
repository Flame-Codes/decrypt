...
def rmbg(file_name):
    ...
    exit("decrypt done!. saved to `%s`" % file_name)
...
def main():
    if len(sys.argv) != 2:
        exit("usage: flame file_name.py")
    globals()['dah_lah']=sys.argv[1]
    sys.argv=[dah_lah,"dec.py",".master_key"]
    print("If You Get Error Decompile, Error dec saved to %s"%sys.argv[1])
    dis(*sys.argv)
